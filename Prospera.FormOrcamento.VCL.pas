unit Prospera.FormOrcamento.VCL;
...
uses
  ...
  cxDBLookupComboBox,
  JvExDBGrids,
  JvDBGrid, RpDefine, RpRave, RpCon, RpConDS, ZAbstractConnection, ZConnection,
  ZAbstractRODataset, ZAbstractDataset, ZDataset, RpBase, RpSystem,
...
type
  ...
  TProsperaFormOrcamento = class(TForm)
    pnlPrincipal: TPanel;
    pnl5: TPanel;
    pnl8: TPanel;
    pgcOSPrincipal: TPageControl;
    ...
    Panel2: TPanel;
    Panel23: TPanel;
    btnStatusOrcamento: TJvTransparentButton;
    btnStatusRecusada: TJvTransparentButton;
    btnStatusAprovada: TJvTransparentButton;
    btnStatusConcluida: TJvTransparentButton;
    Panel40: TPanel;
    Panel41: TPanel;
    lblStatus: TLabel;
    ...
    Label15: TLabel;
    lblOSDesconto: TLabel;
    GridPanel8: TGridPanel;
    Label18: TLabel;
    lblOSRecebido: TLabel;
    GridPanel9: TGridPanel;
    Label21: TLabel;
    lblOSPendente: TLabel;
    ...
    btnOrcamentoLista: TJvTransparentButton;
    ...
    procedure SetarUIStatus(ATag: Integer; ATexto: string);
    ...
    procedure btnOrcamentoListaClick(Sender: TObject);
  private
...
var
  ProsperaFormOrcamento: TProsperaFormOrcamento;
  FOS: TOS;
  ...
  FTrabalhandoEmOrcamento: Boolean = False;
implementation
...
procedure TProsperaFormOrcamento.OnScrollAcabamento(DataSet: TDataSet);
begin
  ListarQuantidade(GetCDS('OrcamentoGF_CategoriaListar').FString('ID'),
    GetCDS('OrcamentoGF_MaterialListar').FString('ID_MATERIAL'),
    GetCDS('OrcamentoGF_CorListar').FString('ID_COR'), GetCDS('OrcamentoGF_CoberturaListar')
    .FString('ID_COBERTURA'), GetCDS('OrcamentoGF_TamanhoListar').FString('ID_TAMANHO'),
    GetCDS('OrcamentoGF_AcabamentoListar').FString('ID_ACABAMENTO'));

end;

procedure TProsperaFormOrcamento.OnScrollCategoria(DataSet: TDataSet);
begin
  ListarMaterial(GetCDS('OrcamentoGF_CategoriaListar').FString('ID'));
end;

procedure TProsperaFormOrcamento.OnScrollCobertura(DataSet: TDataSet);
begin
  ListarTamanho(GetCDS('OrcamentoGF_CategoriaListar').FString('ID'),
    GetCDS('OrcamentoGF_MaterialListar').FString('ID_MATERIAL'),
    GetCDS('OrcamentoGF_CorListar').FString('ID_COR'), GetCDS('OrcamentoGF_CoberturaListar')
    .FString('ID_COBERTURA'));
end;

procedure TProsperaFormOrcamento.OnScrollCor(DataSet: TDataSet);
begin
  ListarCobertura(GetCDS('OrcamentoGF_CategoriaListar').FString('ID'),
    GetCDS('OrcamentoGF_MaterialListar').FString('ID_MATERIAL'),
    GetCDS('OrcamentoGF_CorListar').FString('ID_COR'));
end;

procedure TProsperaFormOrcamento.OnScrollMaterial(DataSet: TDataSet);
begin
  ListarCor(GetCDS('OrcamentoGF_CategoriaListar').FString('ID'),
    GetCDS('OrcamentoGF_MaterialListar').FString('ID_MATERIAL'));
end;

procedure TProsperaFormOrcamento.OnScrollQuantidade(DataSet: TDataSet);
begin
  ListarPrecos;
end;

procedure TProsperaFormOrcamento.OnScrollTamanho(DataSet: TDataSet);
begin
  ListarAcabamento(GetCDS('OrcamentoGF_CategoriaListar').FString('ID'),
    GetCDS('OrcamentoGF_MaterialListar').FString('ID_MATERIAL'),
    GetCDS('OrcamentoGF_CorListar').FString('ID_COR'), GetCDS('OrcamentoGF_CoberturaListar')
    .FString('ID_COBERTURA'), GetCDS('OrcamentoGF_TamanhoListar').FString('ID_TAMANHO'));
end;

procedure TProsperaFormOrcamento.PrepararParaNovoItem;
begin
...
procedure TProsperaFormOrcamento.REMOVERITEM1Click(Sender: TObject);
var
  LNumeroItem,
  LQuantidadeItens,
  I: Integer;
  ...
  begin
    ...
    with
      ComumCDS('OrcamentoGF_CatalogoQuantidadeTabeladaPreco',
    ...
    begin
      ...
      begin
        ...
        begin
          with
            GetCDS('__05__Verificar_Categoria',
          ...
          begin
            ...
            begin
              Result :=
                GetCDS('OrcamentoGF_CatalogoQuantidadeTabeladaPreco').CDS.
...
var
  LSQL: string;
begin
  with GetCDS('OrcamentoGF_OSProdutosListar') do
  begin
    with CDS do
    begin
      if RecordCount = 0 then
      ...
      try
        ...
        begin
          with
            GetCDS('___GFOrcamento_RemoverItem',
    ...
    begin
      with
        GetCDS('_04b_Repassar_Preco_Tabelado',
               'select * from pro_os_linha where  ' +
                   'id_conta = ' + QuotedStr(gSistema.IDConta) + ' and ' +
                   ' ID_OS = ' +
      ...
      begin
        ...
        begin
          ...
          with
            GetCDS('___999C___GetIdCategoria', 'select first(1) * from pro_gf_match ' +
              'where  ' +
                   'id_conta = ' + QuotedStr(gSistema.IDConta) + ' and ' +
          ...
          with
            GetCDS('_203b_Repassar_Preco_Tabelado',
                   'select * from pro_os_linha where  ' +
                       'id_conta = ' + QuotedStr(gSistema.IDConta) + ' and ' +
                       ' ID_OS = ' +
                   QuotedStr(FOS.ID)).CDS do
          begin
            ...
            begin
              ...
              with
                GetCDS('___999D___GetIdCategoria', 'select first(1) * from pro_gf_match ' +
                  'where  ' +
                       'id_conta = ' + QuotedStr(gSistema.IDConta) + ' and ' +
...
procedure TProsperaFormOrcamento.btnStatusAprovadaClick(Sender: TObject);
begin
...
procedure TProsperaFormOrcamento.btnStatusConcluidaClick(Sender: TObject);
begin
  if AusenteClienteDispositivo then
    Exit;
...
procedure TProsperaFormOrcamento.btnStatusCriacaoClick(Sender: TObject);
var
  LStatusStr: string;
  LStatusID: Integer;
  LRecNo: Integer;
begin
  if GetCDS('OrcamentoGF_OSProdutosListar').CDS.Recordcount = 0 then
  begin
  ...
  try
    if ShowMsg('Confirma alterar status do item: ' + #13#10 + #13#10 +
      GetCDS('OrcamentoGF_OSProdutosListar').FString('DESCRICAO') + ' X ' +
      GetCDS('OrcamentoGF_OSProdutosListar').FString('QUANTIDADE') + #13#10 + #13#10 +
      ' para "' + LStatusStr + '"?', True) = 2 then
      Exit;

    LRecNo := GetCDS('OrcamentoGF_OSProdutosListar').CDS.RecNo;

    with ComumCDS('OrcamentoGF_ProducaoOSItemStatusAlterar',
      ['pin_id_conta', 'pin_id_os', 'pin_id_item'],
      [gSistema.IdConta, GetCDS('OrcamentoGF_OSProdutosListar').FString('ID_OS'),
      GetCDS('OrcamentoGF_OSProdutosListar').FString('ID_ITEM')]) do
    begin
      with CDS do
      begin
        ...
        GetCDS('OrcamentoGF_OSProdutosListar').CDS.Refresh;
//        GetCDS('OrcamentoGF_OSProdutosListar').CDS.RecNo := LRecNo;
...
procedure TProsperaFormOrcamento.btnStatusEnviadoProducaoClick(Sender: TObject);
begin
  ShowMsg('Deve ser marcado no módulo "Produção".');
  AfterScrollItem(nil);
end;

procedure TProsperaFormOrcamento.btnAbrirOSClick(Sender: TObject);
var
  LIDAbrir: string;
begin
...
procedure TProsperaFormOrcamento.btnBuscarClick(Sender: TObject);
begin
  ...
  begin
    ...
    begin
      with ComumCDS('OrcamentoOS_ProdutoPrecoDiferenciado',
      ...
      begin
        ...
        begin
          ...
          begin
            with
              ComumCDS('OrcamentoOS_ProdutoValorUnidade',
...
procedure TProsperaFormOrcamento.btnFecharClick(Sender: TObject);
begin
  Self.Close;
end;

procedure TProsperaFormOrcamento.btnSalvarOSClick(Sender: TObject);
begin
...
//    end;
  ...
  begin
    btnPadraoDesbloq.Click;
  end;   }
end;

procedure TProsperaFormOrcamento.btnFiltrarClick(Sender: TObject);
begin
...
procedure TProsperaFormOrcamento.btnLancarClick(Sender: TObject);
var
  LLinhaAtual: Integer;
  LIDLinha: string;
  LPrecoDiferenciado,
...
begin
  ...
  try
    ...
    begin
      with ComumCDS('OrcamentoOS_LancamentoPrecoDiferenciado',
        ['pin_id_conta', 'pin_id_cliente', 'pin_id_produto'],
    ...
    begin
      with ComumCDS('OrcamentoOS_ProdutoPrecos',
      ['pin_id_conta', 'pin_id_Produto'],
    ...
    with ComumCDS('OrcamentoOS_ProdutoRegistrar',
    ...
    begin
      ...
      begin
        ...
        FieldByName('VALOR_TOTAL').AsFloat := FOS.Linhas[LLinhaAtual]
          .ValorTotal;
        FieldByName('ORCAMENTO').AsInteger := 1;
        Post;
      end;
    end;

    with ComumCDS('OrcamentoOS_ProdutosListar',
      ['pin_id_conta', 'pin_id_os'],
      [gSistema.IDConta, FOS.ID]) do
    begin
...
procedure TProsperaFormOrcamento.btnLancarDefeitoClick(Sender: TObject);
begin
...
procedure TProsperaFormOrcamento.btnMaoDeObraAdicionarClick(Sender: TObject);
var
  LLinhaAtual: Integer;
  LIDLinha: string;
...
//    LIDMaoDeObraDescricao := GerarGUID;
    ...
    with ComumCDS('OrcamentoGF_OSItemDescricao') do
    begin
      with CDS do
      begin
        Append;
...
//        FieldByName('ID_PRODUTO').AsString := ''; //FOS.Linhas[LLinhaAtual].IDProduto;
    ...
    with ComumCDS('OrcamentoGF_OSItemStatus') do
    begin
      with CDS do
      begin
        Append;
...
procedure TProsperaFormOrcamento.btnNovaOSClick(Sender: TObject);
begin
  FTrabalhandoEmOrcamento := False;
  ...
  FCriandoNovoOrcamento := True;

  InstanciarOS(True, '');
end;

procedure TProsperaFormOrcamento.btnStatusOrcamentoClick(Sender: TObject);
begin
  if not ConfirmarStatus(Sender) then
    Exit;
...
procedure TProsperaFormOrcamento.btnPadraoDesbloqClick(Sender: TObject);
begin
...
procedure TProsperaFormOrcamento.btnReceberCartaoOSClick(Sender: TObject);
begin
...
procedure TProsperaFormOrcamento.btnReceberDinheiroOSClick(Sender: TObject);
begin
...
procedure TProsperaFormOrcamento.btnRefreshClick(Sender: TObject);
begin
  AtualizarAreaDireita;
end;

procedure TProsperaFormOrcamento.btnReimprimirReciboClick(Sender: TObject);
var
...
begin
  ...
  try
    ...
    with GetCDS('OrcamentoGF_OSProdutosListar').CDS do
    begin
...
procedure TProsperaFormOrcamento.btnStatusRecusadaClick(Sender: TObject);
begin
  if AusenteClienteDispositivo then
    Exit;
...
procedure TProsperaFormOrcamento.CalcularArea;
var
...
begin
  ...
  begin
    ...
    FPrecoCalculado_IDMaterial := dlcMaterial.KeyValue;
  ...
  begin
    ...
    begin
      with
        ComumCDS('OrcamentoGF_OSDescontoMetro',
                 ['pin_id_conta', 'pin_id_produto'],
...
procedure TProsperaFormOrcamento.CalcularQuantidadePreco;
var
...
procedure TProsperaFormOrcamento.cbxAnaliseContatoLiquidoClick(Sender: TObject);
begin
...
procedure TProsperaFormOrcamento.cbxDispositivoEntregueClick(Sender: TObject);
begin
...
procedure TProsperaFormOrcamento.cbxDispositivoRecebidoClick(Sender: TObject);
begin
...
procedure TProsperaFormOrcamento.cbxNaoLigaClick(Sender: TObject);
begin
  with cbxNaoLiga do
  begin
    if Checked then
...
procedure TProsperaFormOrcamento.AfterScrollItem(DataSet: TDataSet);
var
  I: Integer;
  LIndex: Integer;
begin
  ...
  begin
    ...
    for I := 0 to ProsperaFormOrcamento.ComponentCount - 1 do
    begin
      if ProsperaFormOrcamento.Components[I] is TJvTransparentButton then
      begin
        with TJvTransparentButton(ProsperaFormOrcamento.Components[I]) do
        begin
...
procedure TProsperaFormOrcamento.AtualizarAreaDireita;
begin
  with ComumCDS('OrcamentoGF_OSProdutosListar',
  ['pin_id_conta', 'pin_id_os'],
  [gSistema.IDConta, FOS.ID]) do
  begin
    {if not Assigned(dbgGFItens.DataSource) then
...
procedure TProsperaFormOrcamento.AtualizarPagamento;
var
  LDesconto: Real;
begin
  LDesconto := 0;

  with ComumCDS('OrcamentoOS_Desconto',
  ['pin_id_conta', 'pin_id_os'],
  [gSistema.IDConta, FOS.ID]) do
  begin
    with CDS do
  ...
  begin
    ...
    ValorPendente := (ValorTotal - Desconto) - PagoTotal;
  ...
  lblOSTroco.Caption := FormatFloat('###0.00', Pagamento.Troco);
end;

procedure TProsperaFormOrcamento.AtualizarRecebido(APrazo: Boolean = False);
begin
  with Pagamento do
  begin
    if APrazo then
...
function TProsperaFormOrcamento.AusenteClienteDispositivo: Boolean;
var
  I: Integer;
begin
  if ProRec('OS_Dispositivos') then
...
function TProsperaFormOrcamento.ConfirmarStatus(Sender: TObject): Boolean;
var
  I: Integer;
begin
  with TJvTransparentButton(Sender) do
...
procedure TProsperaFormOrcamento.btnSelecionarClienteClick(Sender: TObject);
begin
...
procedure TProsperaFormOrcamento.InstanciarOS(ANovaOS: Boolean; AID_OS: string);
var
  I: Integer;
begin
  ...
  with ComumCDS('OrcamentoOS_Editar',
    ['pin_id_conta', 'pin_id_os'],
...
//  FOS := TOS.Create;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          FOS.NomeCliente := ComumCDS('OrcamentoOS_GetNomeCliente', ['pin_id_cliente'],
  ...
  begin
    ...
    with ComumCDS('OrcamentoOS_Historico', ['pin_id_os'], [FOS.ID]) do
    begin
...
procedure TProsperaFormOrcamento.JvTransparentButton1Click(Sender: TObject);
begin
...
procedure TProsperaFormOrcamento.JvTransparentButton2Click(Sender: TObject);
begin
  btnNovaOS.Click;//ATENCAO
  Self.Close; //ATENCAO
end;

procedure TProsperaFormOrcamento.btnOrcamentoListaClick(Sender: TObject);
var
  LRvProject: TRvProject;
begin
  {if lblNomeCliente.Caption = '' then
  ...
  try
    ...
    begin
      if RecordCount > 5 then
        LRvProject := RvProject4
      else
        LRvProject := RvProject3;
    end;
...
procedure TProsperaFormOrcamento.btnFecharOSClick(Sender: TObject);
var
...
//          ShowMsg('Falha ao descartar OS. ' + E.Message);
    ...
    GetCDS('OrcamentoGF_OSProdutosListar').CDS.Close;
  end
  else
  begin
//    ShowMsg('A OS não foi salva.');
...
procedure TProsperaFormOrcamento.btnGFOSAdicionarItemClick(Sender: TObject);
var
  LLinhaAtual: Integer;
  LIDLinha: string;
  LPrecoVenda: Real;
...
begin
  ...
  try
    ...
    begin
      ...
      LQuantidade := GetCDS('OrcamentoGF_QuantidadeListar').FInteger('DESCRICAO');
    ...
    with ComumCDS('OrcamentoOS_ProdutoRegistrar', ['pin_id_os'], [FOS.ID]) do
    begin
      with CDS do
      begin
        Append;
    ...
    begin
      ...
      begin
        ...
        FieldByName('DESCRICAO').AsString := dlcCategoria.Text + ' ' +
          ...
          '[Acabamento: ' + dlcAcabamento.Text + '] ' + dlcTamanho.Text;
        Post;
      end;
    end;
...
procedure TProsperaFormOrcamento.LimparTudo;
begin
...
procedure TProsperaFormOrcamento.btnDescartarOSClick(Sender: TObject);
begin
  if FOS.IDCliente <> '' then
...
procedure TProsperaFormOrcamento.mmoDefeitoRelatadoChange(Sender: TObject);
begin
  FOS.DefeitoRelatado := mmoDefeitoRelatado.Text;
end;

procedure TProsperaFormOrcamento.mmoObservacoesOSChange(Sender: TObject);
begin
  FOS.ServicoNecessario := mmoObservacoesOS.Text;
end;

procedure TProsperaFormOrcamento.DestruirObjetos;
begin
//  FOS.Destroy;
end;

procedure TProsperaFormOrcamento.dlcDispositivoPropertiesChange(Sender: TObject);
begin
  if dlcDispositivo.Text <> '' then
...
procedure TProsperaFormOrcamento.dlcDispositivoChange(Sender: TObject);
begin
  if dlcDispositivo.Text <> '' then
    FOS.IDDispositivo := dlcDispositivo.KeyValue
  else
...
procedure TProsperaFormOrcamento.dtpDataPrevisaoChange(Sender: TObject);
begin
  FOS.DataPrevisao := dtpDataPrevisao.Date;
end;

procedure TProsperaFormOrcamento.dtpHoraPrevisaoChange(Sender: TObject);
begin
  FOS.HoraPrevisao := dtpHoraPrevisao.Time;
end;

procedure TProsperaFormOrcamento.edtAlturaChange(Sender: TObject);
begin
  CalcularArea;
end;

procedure TProsperaFormOrcamento.edtBaseChange(Sender: TObject);
begin
  CalcularArea;
end;

procedure TProsperaFormOrcamento.edtBuscarChange(Sender: TObject);
var
  LTextoBusca: string;
  LNomeCliente: string;
  LNumeroOS: Integer;
...
procedure TProsperaFormOrcamento.edtQuantidadeDinamicaChange(Sender: TObject);
begin
  CalcularArea;
  //CalcularQuantidadePreco;
end;

procedure TProsperaFormOrcamento.FiltrarOS(ANomeCliente: string = '';
  ANumeroOS: Integer = 0);
begin
  ...
  with
    ComumCDS('OrcamentoOS_AbertasListar',
...
procedure TProsperaFormOrcamento.FormCreate(Sender: TObject);
begin
  tabNovaOrdem.TabVisible := False;
  pgcOSPrincipal.ActivePage := tabListaOS;
...
procedure TProsperaFormOrcamento.FormShow(Sender: TObject);
begin
  FiltrarOS('', 0);
end;

procedure TProsperaFormOrcamento.NovoHistorico(ATexto: string);
begin
  with ComumCDS('OrcamentoOS_Historico', ['pin_id_os'], [FOS.ID]) do
  begin
    with dbgHistoricoOS do
...
function TProsperaFormOrcamento.NovoNumeroOS: LongInt;
begin
  Result := 0;
end;

procedure TProsperaFormOrcamento.OnClickStatus(Sender: TObject);
var
  LButton: TJvTransparentButton;
begin
  LButton := TJvTransparentButton(Sender);

  with ComumCDS('OrcamentoOS_StatusAtualizar', ['pin_id_os'], [FOS.ID]) do
  begin
    with CDS do
    begin
      if Recordcount = 0 then
...
function TProsperaFormOrcamento.SalvarOS: Boolean;
begin
  Result := False;
  ...
  try
    ...
    begin
      with CDS do
      begin
        if FCriandoNovoOrcamento then
        begin
          Append;
          FieldByName('ID').AsString := FOS.ID;
          FCriandoNovoOrcamento := False;
        end
        else
          Edit;
...
procedure TProsperaFormOrcamento.SetarUIStatus(ATag: Integer; ATexto: string);
var
  LStatusOS: string;
begin
  ...
  lblNumeroOrdem.Caption := 'Orçamento'; 
end;

procedure TProsperaFormOrcamento.dlcAcabamentoChange(Sender: TObject);
begin
...
procedure TProsperaFormOrcamento.dlcCategoriaChange(Sender: TObject);
begin
  pnlInformarQuantidade.Visible := False;
  pnlCalcularArea.Visible := False;
...
procedure TProsperaFormOrcamento.dlcCorChange(Sender: TObject);
begin
  dlcCobertura.ClearValue;
  dlcTamanho.ClearValue;
  dlcQuantidade.ClearValue;
...
procedure TProsperaFormOrcamento.dlcCoberturaChange(Sender: TObject);
begin
  dlcTamanho.ClearValue;
  dlcAcabamento.ClearValue;
  dlcQuantidade.ClearValue;
...
procedure TProsperaFormOrcamento.dlcMaterialChange(Sender: TObject);
begin
  dlcCor.ClearValue;
  dlcCobertura.ClearValue;
  dlcTamanho.ClearValue;
...
procedure TProsperaFormOrcamento.dlcQuantidadeChange(Sender: TObject);
begin
  dlcQuantidade.Visible := True;
  lblQuantidade.Visible := True;
  ...
  begin
    ...
    FValor := GetCDS('OrcamentoGF_OSPrecoVenda').CDS.FieldByName('VALOR').AsFloat;
    ...
    if GetCDS('OrcamentoGF_QuantidadeListar').FString('DESCRICAO') = '1' then
    begin
...
procedure TProsperaFormOrcamento.dlcTamanhoChange(Sender: TObject);
begin
...
procedure TProsperaFormOrcamento.ListarAcabamento(ACategoria, AMaterialID, ACorID,
  ACoberturaID, ATamanhoID: string);
begin
  with ComumCDS('OrcamentoGF_AcabamentoListar', ['pin_id_categoria', 'pin_id_material',
...
procedure TProsperaFormOrcamento.ListarCategoria;
begin
  with ComumCDS('OrcamentoGF_CategoriaListar') do
  begin
    with CDS do
    begin
      DisableControls;
...
procedure TProsperaFormOrcamento.ListarCobertura(ACategoria, AMaterialID,
  ACorID: string);
begin
  with ComumCDS('OrcamentoGF_CoberturaListar', ['pin_id_categoria', 'pin_id_material',
...
procedure TProsperaFormOrcamento.ListarCor(ACategoria, AMaterialID: string);
begin
  with ComumCDS('OrcamentoGF_CorListar', ['pin_id_categoria', 'pin_id_material'],
...
procedure TProsperaFormOrcamento.ListarMaterial(ACategoria: string);
begin
  with ComumCDS('OrcamentoGF_MaterialListar', ['pin_id_categoria'], [ACategoria]) do
  begin
    with CDS do
    begin
      DisableControls;
...
procedure TProsperaFormOrcamento.ListarPrecos;
begin
  FMatchID := GetCDS('OrcamentoGF_QuantidadeListar').FString('ID');

  with ComumCDS('OrcamentoGF_OSPrecoVenda', ['pin_id_match'], [FMatchID]) do
  begin

  end;
end;

procedure TProsperaFormOrcamento.ListarQuantidade(ACategoria, AMaterialID, ACorID,
  ACoberturaID, ATamanhoID, AAcabamentoID: string);
begin
  with ComumCDS('OrcamentoGF_QuantidadeListar', ['pin_id_categoria', 'pin_id_material',
...
procedure TProsperaFormOrcamento.ListarTamanho(ACategoria, AMaterialID, ACorID,
  ACoberturaID: string);
begin
  with ComumCDS('OrcamentoGF_TamanhoListar', ['pin_id_categoria', 'pin_id_material',
